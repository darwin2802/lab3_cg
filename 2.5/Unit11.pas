...
procedure TForm1.DrawDoubleCone;
const
  n = 200;
  h = 2.0;
  r = 1.0;
var
  ...
  rTop, gTop, bTop, rBottom, gBottom, bBottom: GLfloat;
  x, y: GLfloat;
begin
  dfi := 2 * Pi / n;

  rTop := 0.9 + 0.1 * Sin(ColorPhase + Pi/3);
  gTop := 0.3 + 0.5 * Sin(ColorPhase + 2*Pi/3);
  bTop := 1.0;

  rBottom := 0.0 + 0.5 * Sin(ColorPhase);
  gBottom := 0.8 + 0.2 * Sin(ColorPhase + Pi/2);
  bBottom := 0.7 + 0.3 * Sin(ColorPhase + Pi);
