...
procedure TForm1.DrawTransformedCylinder;
const
  n = 200;
  baseHeight = 3.0;
  baseRadius = 1.0;
var
  ...
  radiusFactor, heightFactor: GLfloat;
begin
  dfi := 2 * Pi / n;
  ...
  begin
    ...
    radiusFactor := 0.8 + 0.2 * Sin(fi*4 + TimePhase);
    heightFactor := 0.5 + 0.5 * Sin(TimePhase + fi*2);

    x := baseRadius * radiusFactor * Cos(fi);
    y := baseRadius * radiusFactor * Sin(fi);
    z := -baseHeight/2 * heightFactor;
